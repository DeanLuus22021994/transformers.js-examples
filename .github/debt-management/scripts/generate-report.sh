...
REPORT_FILE="debt-weekly-report.md"
...
# Get basic stats
TOTAL_DEBT=$(grep "Total debt items found:" debt-report.md | sed 's/Total debt items found: \(.*\)/\1/')
...
sed -n '/^## Debt Markers Found/,/^## Summary/p' debt-report.md | grep '###' | \
while read -r line; do
    type=$(echo "$line" | sed 's/### \(.*\) Items/\1/')
    count=$(sed -n "/### $type Items/,/###/p" debt-report.md | grep -c "| " | expr - 2)
...
echo "" >> "$REPORT_FILE"
sed -n '/^## Debt by Directory/,/^$/p' debt-report.md | sed '1,3d' >> "$REPORT_FILE"
