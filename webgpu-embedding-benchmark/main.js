...
if (!gpuHasFp16) {
  const element = document.querySelector(
    '.tests[data-device="webgpu"][data-dtype="fp16"]',
  );
...
start.addEventListener("click", async () => {
  const validTests = [...tests].filter(
    (test) => !test.getAttribute("unsupported"),
  );
  // Update UI
  start.disabled = true;
  stop.disabled = false;
  ...
  const batch_sizes = batchSizes.value
    .split(",")
    .map((x) => parseInt(x))
    .filter((x) => x);
...
function generateResultsTable(model_id, testNames, data, sequence_length) {
  ...
  info.appendChild(
    createBulletPoint(
      `GPU: vendor=${adapterInfo.vendor}, architecture=${adapterInfo.architecture}, device=${adapterInfo.device}, description=${adapterInfo.description}`,
    ),
  );
